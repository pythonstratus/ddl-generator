...
echo "  Rewrote $REF_BEFORE reference(s)"
echo
...
STEP_DIRS=(sequences tables indexes procedures views synonyms)
...
for d in "${STEP_DIRS[@]}"; do
    ...
    case ${#hits[@]} in
        ...
        0) echo "  NO MATCH        ${d}/  (expected ${SRC_USER}_<Type>Prod.sql)"
           RESOLVED+=("") ; FAILED=1 ;;
...
if [[ -d "$DST/functions" || -d "$DST/materialized_views" ]]; then
    echo
    echo "  NOTE: functions/ and/or materialized_views/ exist but are not"
    echo "        part of the six-step sequence, matching the original"
    echo "        driver. They are emitted as commented-out steps. If they"
    echo "        are in scope, uncomment them -- and materialized views"
    echo "        also need GRANT CREATE MATERIALIZED VIEW TO ${DST_USER}."
fi
...
BEGIN
    ...
    IF v_count > 0 THEN
        RAISE_APPLICATION_ERROR(-20002,
            'ABORTED: ENTITYDEV holds ' || v_count || ' object(s). Run ' ||
            '99_entitydev_teardown.sql first, or comment out this guard.');
    END IF;
...
-- =====================================================================
-- STEP $n OF 6 : ${STEP_LABELS[$i]}
-- =====================================================================
PROMPT
PROMPT ================================================================
PROMPT  STEP $n OF 6 : ${STEP_LABELS[$i]}
PROMPT  Source      : ${d}/${f}
PROMPT ================================================================
PROMPT
...
EOS
    ...
    if [[ "$d" == "procedures" && -d "$DST/functions" ]]; then
        echo "" >> "$MASTER_OUT"
        echo "-- functions/ exists but was not part of the original six-step" >> "$MASTER_OUT"
        echo "-- sequence. Uncomment if in scope, and check the filename." >> "$MASTER_OUT"
        echo "-- @@functions/${SRC_USER}_FunctionsProd.sql" >> "$MASTER_OUT"
    fi
