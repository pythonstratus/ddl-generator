...
stateDiagram-v2
    ...
    note left of Pending
        The Group Summary Priority 99 count
...
```mermaid
flowchart LR
    subgraph BEFORE["Before selection"]
        B1["Queued: 46"]
        B2["Listed: 46"]
        B3["Group Summary Priority 99: 46"]
        B4["Group Summary Pending: 0"]
    end

    EVENT{{"Manager selects<br/>2 accelerated cases"}}

    subgraph AFTER["After selection"]
        A1["Queued: 44 — DECREASES"]
        A2["Listed: 46 — unchanged"]
        A3["Group Summary Priority 99: 46 — unchanged"]
        A4["Group Summary Pending: 2 — INCREASES"]
    end

    BEFORE --> EVENT --> AFTER

    classDef moves fill:#fff4e5,stroke:#e65100,color:#000
    classDef holds fill:#eceff1,stroke:#546e7a,color:#000
    class A1,A4 moves
    class A2,A3 holds
```
