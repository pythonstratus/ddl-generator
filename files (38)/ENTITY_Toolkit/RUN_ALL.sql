...
--
--   If ENTITY_DDLs_DryRun is missing you will get, at the very last
--   step, after everything else has succeeded:
--
--       SP2-0310: unable to open file
--                 "ENTITY_DDLs_DryRun/master_run_entitydev.sql"
--
--   That means no DDL ran at all. Request the full package rather than
--   the loose SQL files.
--
-- ---------------------------------------------------------------------
-- HOW TO RUN
...
--   skipped. The DDL contains DROP TABLE statements, and connecting as
--   ENTITYDEV is what confines them to the sandbox.
--
-- ---------------------------------------------------------------------
-- PASSWORDS -- READ THIS
--
--   This script does NOT set any password. It only asks for the
--   existing ENTITYDEV password so it can reconnect.
--
--   That is deliberate. When an ALTER USER ... IDENTIFIED BY statement
--   fails, SQL*Plus echoes the statement back with the password already
--   substituted in, so the value lands in the log in clear text. Not
--   issuing the statement removes the exposure entirely.
--
--   IF YOU NEED TO CHANGE THE PASSWORD, do it at the SQL> prompt as
--   SYSDBA, before running this script:
--
--       SQL> PASSWORD ENTITYDEV
--
--   The PASSWORD command prompts without echo and never places the
--   value in a statement, so it cannot appear in a log even on failure.
--   Do not use ALTER USER ... IDENTIFIED BY for this.
--
--   ORA-28007 "password cannot be reused" means the profile keeps a
--   password history. Choose a value that has not been used before.
--
-- ---------------------------------------------------------------------
-- LOGS TO SEND BACK
--
...
--
--   If the run stops, send the exact ORA- or SP2- number and the step
--   it stopped at rather than working around it.
-- =====================================================================
...
-- Produces entitydev_prepare.log. Sets no password.
-- =====================================================================
PROMPT
PROMPT ===== STEP 3 of 4 : Unlocking and adjusting privileges =====
PROMPT
...
--
-- Connecting through the service name lands in the correct PDB, so no
-- container switch is needed here.
--
